...
const Admins = ({ navigation }) => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      let res = await axiosInstance.get("/users/admins");
  ...
  const makeUser = async (user) => {
    try {
      const axiosInstance = await generateAxiosInstance(true);
      let res = await axiosInstance.post("/users/make-user", { id: user.id });
          ...
          return (
            ...
            >
              ...
              <View style={{
                ...
                <Pressable onPress={() => makeUser(user)} style={{ padding: 2, margin: 2, backgroundColor: '#e1e1e1' }}>
                  <Text style={{ fontSize: 12, textAlign: 'center' }}>User</Text>
                </Pressable>
