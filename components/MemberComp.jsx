...
const MemberComp = () => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      await Promise.all([
        fetchPlacesData(), 
        fetchCurrencies(), 
        fetchAdminUsers() // Add admin users fetch
      ]);
    } catch (error) {
  ...
  // Modified function to handle chat navigation - only with admins
  const handleChatPress = (order) => {
    // Check if there are any admin users available
    ...
    // If there's only one admin, chat with them directly
    if (adminUsers.length === 1) {
      const admin = adminUsers[0];
      navigation.navigate("ChatScreen", {
        orderId: order.id,
        targetUserId: admin.id,
        targetUserName: admin.full_name,
        isOrderChat: true,
      });
      return;
    }

    // If there are multiple admins, show selection modal
    // For now, we'll just select the first admin (you can modify this to show a selection modal)
    const firstAdmin = adminUsers[0];
    navigation.navigate("ChatScreen", {
      orderId: order.id,
      targetUserId: firstAdmin.id,
      targetUserName: firstAdmin.full_name,
      isOrderChat: true,
    });
  };

  // Optional: Add a direct chat with admin function (not order-specific)
  const handleDirectAdminChat = () => {
    ...
    if (adminUsers.length === 1) {
      ...
      navigation.navigate("ChatScreen", {
        targetUserId: admin.id,
        targetUserName: admin.full_name,
        isOrderChat: false, // This is a direct chat, not order-specific
      });
      return;
    }

    // If multiple admins, select the first one (you can modify this logic)
    const firstAdmin = adminUsers[0];
    navigation.navigate("ChatScreen", {
      targetUserId: firstAdmin.id,
      targetUserName: firstAdmin.full_name,
